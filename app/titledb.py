...
# Serializes concurrent update runs (scheduled job, missing-file recovery,
# settings save). Without it, two runs downloaded into the same fixed .tmp
# paths and corrupted each other's files. A run that waited here typically
# finds everything up to date and becomes a cheap no-op.
